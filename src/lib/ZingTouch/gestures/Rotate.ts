...
class Rotate extends Gesture {
  ...
  constructor(options = {}) {
     ...
     * @type {String}
     */
    // @ts-expect-error ts-migrate(2339) FIXME: Property 'type' does not exist on type 'Rotate'.
    this.type = 'rotate';
     ...
     */
    // @ts-expect-error ts-migrate(2339) FIXME: Property 'onMove' does not exist on type '{}'.
    if (options && options.onMove && typeof options.onMove === 'function') {
      // @ts-expect-error ts-migrate(2551) FIXME: Property 'onMove' does not exist on type 'Rotate'.... Remove this comment to see the full error message
